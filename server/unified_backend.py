...
class UnifiedRobotBackend(RobotBackend):
    ...
    def send_target_pose(self, position: np.ndarray, orientation: np.ndarray, velocity_limit: float = 0.1, gripper_state: float = -1.0, handedness: str = "right") -> bool:
        if self._backend:
            return self._backend.send_target_pose(position, orientation, velocity_limit, gripper_state, handedness)
        return False
